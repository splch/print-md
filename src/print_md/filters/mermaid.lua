-- mermaid.lua — Render Mermaid diagrams to SVG via mmdc (mermaid-cli)
--
-- Finds code blocks with class "mermaid", renders them via mmdc,
-- and replaces them with Image elements pointing to the generated SVG.
...
function CodeBlock(el)
  ...
  local output_file = tmp_base .. ".svg"
  ...
  -- Run mmdc to render SVG
  local cmd = string.format(
    'mmdc -i "%s" -o "%s" -b transparent --quiet 2>&1',
  ...
  local svg_check = io.open(output_file, "r")
  if not svg_check then
  ...
  -- Create Image element pointing to the SVG file
